...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'safeguard', handle: 'which which what' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('exactly');
      cy.get(`[data-cy="name"]`).should('have.value', 'exactly');

      cy.get(`[data-cy="handle"]`).type('ecliptic lox');
      cy.get(`[data-cy="handle"]`).should('have.value', 'ecliptic lox');
